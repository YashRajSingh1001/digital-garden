  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16 animate-[fade-in_0.4s_ease-out_both]">
      <div className="mb-12">
        ...
        <p className="text-[#64748b]">Things I&apos;ve built or am currently building.</p>
      </div>

      {projects.length === 0 && (
        <p className="text-[#64748b] text-sm">Add projects in the /projects directory.</p>
      )}
      ...
      <div className="space-y-4">
        {projects.map((p) => {
          ...
          return (
            <div
              key={p.slug}
              className="rounded-xl border border-[#1e2a45] bg-[#0d1224] p-5 hover:border-[#7c3aed]/40 transition-colors"
            >
              ...
              <p className="text-sm text-[#64748b] leading-relaxed mb-3">{p.description}</p>
              {p.stack.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {p.stack.map((tech) => (
                    <span
                      key={tech}
                      className="text-[10px] px-2 py-0.5 rounded bg-[#131929] text-[#64748b] border border-[#1e2a45]"
                    >
                      {tech}
                    </span>
                  ))}

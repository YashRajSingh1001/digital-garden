...
function StatBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[10px] text-[#6b7fa3] uppercase tracking-widest">{label}</span>
      ...
      {lastActivity && (
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex gap-4 shrink-0">
            ...
            <StatBox label="pace" value={formatPace(lastActivity.distance, lastActivity.moving_time)} />
          </div>
        </div>
      )}
      {stats && (
